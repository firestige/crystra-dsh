...
import { parseCrystraCommand, promptDiagnostic } from "./command.js";
import {
...
async function ensureGitHead(workspace) {
  if (!await hasGitHead(workspace)) {
    ...
    await runGit(workspace, [
      "-c", "user.name=CRYSTRA Workspace Initializer",
      "-c", "user.email=crystra@localhost",
      "-c", "commit.gpgSign=false",
      "commit", "--quiet", "--allow-empty", "--no-verify", "-m", "Initialize CRYSTRA workspace",
    ]);
  }
...
export function consumeCrystraCommandBeforeModel(payload) {
...
export async function apply(ctx, config, hooks = {}) {
  ...
  const preStep = ctx.on?.("agent/pre-step", async (payload, next) => {
    ...
    if (consumeCrystraCommandBeforeModel(payload)) return { kind: "reject" };
