...
export function createWorkflowResourceAuthoring({root,workflow,writeAllowed=false,loadCurrent}){
 ...
 const options={root:join(root,namespace),resources};
 ...
 return {
  ...
  async readRevision(resourceId,path,revision){await validate();return createDraftResourceAuthoring({...options,getContext:validate}).readRevision(resourceId,path,revision);},
 };
}
