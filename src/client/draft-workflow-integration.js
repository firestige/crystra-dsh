...
export function createDraftWorkflowIntegration({React,Core,gateway,renderMarkdown,quote,canQuote=()=>false,adapter=createDraftWorkflowAdapter({gateway})}){
 ...
 function Resources({item,reference}){
  ...
  return React.createElement(Core.WorkflowResourceViewer,{definitionId:item.context.definitionId,revision:item.context.definitionRevision,workspace:state.workspace,catalog,renderMarkdown,onDiscuss:discuss,onSaveDraft:p.resourceWriteAllowed===true?store.save:undefined});
 }
