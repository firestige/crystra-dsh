...
export function createDraftWorkflowIntegration({React,Core,gateway,renderMarkdown,quote,canQuote=()=>false,adapter=createDraftWorkflowAdapter({gateway})}){
 ...
 function Resources({item,reference}){
  ...
  return React.createElement('div',{style:{display:'flex',flexDirection:'column',height:'100%',minHeight:0}},notification?React.createElement('p',{role:'status',style:{margin:'4px 12px',fontSize:12}},notification):null,state.notificationError?React.createElement('p',{role:'alert'},'通知投递失败，草案已保存；可重试。'):null,p.resourceNotificationsAllowed===true&&state.workspace.files.some(f=>f.notification?.status==='pending'||['canceled','unavailable'].includes(f.notification?.deliveryState))?React.createElement(Core.Button,{disabled:state.notifying===true,onClick:()=>{void store.retryNotifications().catch(()=>{});}},state.notifying?'正在投递通知':'重试通知'):null,React.createElement('div',{style:{flex:1,minHeight:0}},React.createElement(Core.WorkflowResourceViewer,{renderRelations,definitionId:item.context.definitionId,revision:item.context.definitionRevision,workspace:state.workspace,catalog,renderMarkdown,onDiscuss:discuss,onSaveDraft:p.resourceWriteAllowed===true?store.save:undefined})));
 }
