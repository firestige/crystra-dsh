...
export function admitWorkflowDraft(projection,context,now=Date.now()){
 ...
 if(!['resourceStoreAvailable','resourceWriteAllowed','resourceNotificationsAllowed'].every(k=>optional(projection[k],v=>typeof v==='boolean'))||(projection.resourceWriteAllowed===true&&projection.resourceStoreAvailable!==true)||(projection.resourceNotificationsAllowed===true&&projection.resourceWriteAllowed!==true))return invalid('RESOURCE_AUTHORITY_INVALID');
